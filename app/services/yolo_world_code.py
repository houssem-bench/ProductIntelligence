...
def yolo_box_filter_reason(
    ...
    """Return rejection reason for a detection box, or None if it passes filters."""
    w = max(1, x2 - x1)
    h = max(1, y2 - y1)
    area = w * h
...
class YOLOWorldProductWorkflow:
    """Detect products with YOLO-World, then crop detections and persist metadata."""
    ...
    def _detect_with_yolo_world(self, image) -> List[Dict[str, object]]:
        ...
        print("Using YOLO-World detection")
    ...
    def analyze_image(self, image_path: str) -> ImageAnalysisResult:
        ...
        for detection in detections:
            ...
            product = ProductObject(
                ...
                extra={
                    "detector": "yolo_world" if self.model is not None else "contour_fallback",
                    "model": self.model_name,
                    "classes": ", ".join(self.world_classes),
                },
            )
...
def main() -> None:
    image_path = "ImageC/images.jpg"

    workflow = YOLOWorldProductWorkflow(
        model_name="yolov8l-world.pt",
        world_classes=["food product", "package", "bottle", "can", "box", "carton", "jar", "pouch","packet"],
    )
